...
private:
    ...
    int collect_write_data(client_info *c, char *last_buffer, int last_bytes, int tot_bytes); // SERVER_CMD_OUT_WRITE collect data
    int wait_helper_write(void);            // wait until helper finished writing

public:
    // constructor and destructor
    FPGA_server(unsigned flags, const char *IP, const char *port, int num_cpu); // start server with given flags, and IP:port
    ~FPGA_server();
    ...
    virtual void onSendFinished(client_info *c, void *data, int num, int sent, int error); // sending of large data finished
...
// ONDATA_USER_BITS used by onData
#define ONDATA_ACK          1               // return ACK
#define ONDATA_NACK         2               // return NACK
#define ONDATA_OK           4               // valid command received: enter switch-case
