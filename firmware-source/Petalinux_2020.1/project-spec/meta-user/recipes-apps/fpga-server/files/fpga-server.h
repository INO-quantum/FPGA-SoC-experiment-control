...
public:
    ...
    FPGA_server(unsigned flags, const char *IP, const char *port, int num_cpu, uint32_t strb_delay, uint32_t sync_wait, uint32_t sync_phase); // start server with given flags, and IP:port
    ~FPGA_server();
