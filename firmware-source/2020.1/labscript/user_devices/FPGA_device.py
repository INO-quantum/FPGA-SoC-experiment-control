...
from labscript import (
    ...
    DDS, DigitalOut, AnalogOut, Output, DDSQuantity,
...
if False:
    # this is for DAC712 with 0x0 = 0V. see test_word.
    def V_to_word(volts):
    ...
    # this is for other DACs with 0x0 = -10V
    def V_to_word(volts):
        "convert voltage (scalar or np.array) into low 16 bits of 32bit data word."
        return np.array(np.round((np.clip(volts, AO_MIN, AO_MAX) - AO_MIN) * (2 ** AO_BITS - 1) / (AO_MAX - AO_MIN)), dtype=np.uint32) & DATA_MASK

    def word_to_V(word):
        "convert 16bit data word (scalar or np.array) into voltage"
        return np.array(word, dtype=float) * (AO_MAX - AO_MIN) / (2 ** AO_BITS - 1) + AO_MIN
...
# DDS address offsets and invalid values
...
class SpecialIM(IntermediateDevice):
    ...
    def to_word(self, channel):
        "return data word for this channel"
        return np.array(np.where(channel.raw_output == channel.default_value, 0,
                                 channel.raw_output),
                        dtype=np.uint32)
...
class DigitalChannels(IntermediateDevice):
    ...
    def to_word(self, channel):
        "return data word for this channel"
        return np.array(np.where(channel.raw_output == channel.default_value,
                                 BIT_NOP_SH,
                                 (channel.raw_output*channel.ch_bits) | channel.addr_bits),
                        dtype=np.uint32)
...
class AnalogChannels(IntermediateDevice):
    ...
    # define conversion function from raw data to data word.
    # will be called by generate_code for each AnalogOut device.
    def to_word(self, channel):
        "return data word for this channel"
        return np.array(np.where(channel.raw_output == channel.default_value,
                                 BIT_NOP_SH,
                                 V_to_word(channel.raw_output) | channel.addr_bits),
                        dtype=np.uint32)
...
class DDSChannels(IntermediateDevice):
    ...
    # define conversion functions from raw data to data word.
    # will be called by generate_code for each freq, amp, phase channel of DDS device.
    def to_word(self, channel):
        "return data word for this channel"
        if channel.name.endswith('_freq'):
            return np.array(np.where(channel.raw_output == channel.default_value,
                                     BIT_NOP_SH,
                                     V_to_word(channel.raw_output) | channel.addr_bits),
                            dtype=np.uint32)
        elif channel.name.endswith('_amp'):
            return np.array(np.where(channel.raw_output == channel.default_value,
                                     BIT_NOP_SH,
                                     V_to_word(channel.raw_output) | channel.addr_bits),
                            dtype=np.uint32)
        elif channel.name.endswith('_phase'):
            return np.array(np.where(channel.raw_output == channel.default_value,
                                     BIT_NOP_SH,
                                     V_to_word(channel.raw_output) | channel.addr_bits),
                            dtype=np.uint32)
        else:
            raise LabscriptError("DDS channel '%s' to_word: unknown channel type?" % (channel.name))

    def add_device(self, device):
        save_print("DDSChannels: add device '%s' type '%s'" % (device.name, str(type(device))))
        #check if valid device and channel and if channel is already used
        #create container class and check if valid device, channel and if channel is already used
        #each channel has its own address.
        device.rack, device.address, device.channel = check_device(device, DDS, self.max_channels, self.child_devices, False)
        device.type = TYPE_DDS
        if device.address & 3 != 0:
            # address must be integer multiple of 4 so we can address each sub-channel individually
            raise LabscriptError("device '%s' address 0x%x must be multiple of 4!" % (device.name, device.address))
        device.frequency.type           = TYPE_DDS
        device.frequency.address        = device.address+DDS_FREQ_ADDR_OFFSET
        device.frequency.addr_bits      = np.array((device.frequency.address)<<ADDR_SHIFT,dtype=np.uint32)
        device.frequency.rack           = device.rack
        device.frequency.channel        = 0
        device.frequency.set_property('address', '%i/%i/%i'%(device.frequency.rack,device.frequency.address,device.frequency.channel), 'connection_table_properties')
        device.frequency.user_default_value = device.frequency.default_value
        device.frequency.default_value  = DDS_FREQ_INVALID_VALUE
        device.amplitude.type           = TYPE_DDS
        device.amplitude.address        = device.address+DDS_AMP_ADDR_OFFSET
        device.amplitude.addr_bits      = np.array((device.amplitude.address)<<ADDR_SHIFT,dtype=np.uint32)
        device.amplitude.rack           = device.rack
        device.amplitude.channel        = 1
        device.amplitude.user_default_value = device.amplitude.default_value
        device.amplitude.default_value  = DDS_AMP_INVALID_VALUE
        device.amplitude.set_property('address', '%i/%i/%i'%(device.amplitude.rack,device.amplitude.address,device.amplitude.channel), 'connection_table_properties')
        device.phase.type               = TYPE_DDS
        device.phase.address            = device.address+DDS_PHASE_ADDR_OFFSET
        device.phase.addr_bits          = np.array((device.phase.address)<<ADDR_SHIFT,dtype=np.uint32)
        device.phase.rack               = device.rack
        device.phase.channel            = 2
        device.phase.user_default_value = device.phase.default_value
        device.phase.default_value      = DDS_PHASE_INVALID_VALUE
        device.phase.set_property('address', '%i/%i/%i'%(device.phase.rack,device.phase.address,device.phase.channel), 'connection_table_properties')
        #device.user_default_value = device.default_value # default_value given in connection_table
        #device.default_value = AO_INVALID_VALUE # automatically inserted value by labscript
        #device.id = get_ID(type=device.type, rack=device.rack, address=device.address, channel=device.channel)
        IntermediateDevice.add_device(self, device)
            ...
            for clockline in pseudoclock.child_devices:
                ...
                for IM in clockline.child_devices:
                    ...
                    if isinstance(IM, SpecialIM):
                        special = True
                        continue
                    elif isinstance(IM, DDSChannels):
                        IMlist = IM.child_devices
                    else:
                        IMlist = [IM]

                    # go through all DDS or IM devices
                    for subIM in IMlist:
                        if IM.shared_address: # shared address like digital out
                            ...
                            for dev in subIM.child_devices:
                    ...
                    for IM in clockline.child_devices:
                        if isinstance(IM, SpecialIM):
                            for dev in IM.child_devices:
                                ...
                                data[i,dev.rack+1] |= np.where(changes[i, dev.rack], d[mask], d[mask] | BIT_NOP_SH)
...
class FPGA_Worker(Worker):
    def init(self):
        ...
        AO_MAX = 10.0-20/2**16
        Volt = [-10.1,-10,-9.99,-5,-3.14,0,2.718,5,9.99,10.0,10.1]
        for V in Volt:
            w = V_to_word(V)
            v = word_to_V(w)
            print("V_to_word(%10.6f) = 0x%04x, word_to_V(0x%04x) = %10.6f, error %10.6f" % (V, w, w, v, v-V))
            ...
            try:
                ...
                try:
                    ...
                    if self.first_time or (value != last):
                        ...
                        sample[rack+1] = (address<<ADDR_SHIFT)|V_to_word(value)
...
def test_word():
    ...
    see Texas Instruments, DAC712 datasheet, July 2009, Table 1, page 13
    """
