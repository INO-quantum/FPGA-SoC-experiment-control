...
# runviewer options
runviewer_show_all       = True             # if True show also unused channels
...
CTRL_IN_SRC_DATA_28         = 6                     # data bits 28-31
...
in_dests        = {STR_TRIG_START       : (CTRL_IN_DST_START, CTRL_IN_SRC_IN_ALL, CTRL_IN_LVL_IN_ALL),
                   ...
                   STR_BIT_IRQ          : (CTRL_IN_DST_DATA_IRQ, CTRL_IN_SRC_DATA_ALL, CTRL_IN_LVL_DATA_ALL)}
            ...
            for pseudoclock in self.child_devices:
                for clockline in pseudoclock.child_devices:
                    ...
                    for IM in clockline.child_devices:
                        if isinstance(IM, SpecialIM):
                            for dev in IM.child_devices:
                                #print(dev.name, t, dev.raw_output)
                                #if np.count_nonzero(dev.raw_output == dev.default_value) != len(t):
                                #    print('%s instr:'%dev.name, dev.instructions)
                                #    print('%s raw data:' % dev.name, dev.raw_output)
                                ...
                                #print(mask)
                                #print(t[mask])
                                #print(np.transpose([indices, t, mask]))#, oc, changes[:, dev.rack], od, data[:, dev.rack]]))
...
class RunviewerClass(object):
    ...
    def __init__(self, path, device):
        try:
            ...
            self.type = None
            save_print("\nrunviewer loading '%s'" % (device.name))
    ...
    def get_traces(self, add_trace, clock=None):
        try:
            ...
            if len(data) == 0:
                save_print("'%s' add trace (type %d) no data!" % (self.name, self.type))
            else:
                #save_print('matrix\n',data)
                if self.type == TYPE_board: # main board
                        ...
                        if pseudoclock.device_class == 'FPGA_PseudoClock':
                            for clockline in pseudoclock.child_list.values():
                                if clockline.device_class == 'ClockLine':
                                    ...
                                    add_trace(clockline.name, (time, value), None, None) # this shows clockline also as trace
                                    traces[clockline.name] = (time, value) # this creates RunviewerClass for clockline and calls get_traces for all IM channels
                        elif pseudoclock.device_class == 'Trigger':
                            print('adding Trigger %s' % (pseudoclock.name))
                            traces[pseudoclock.name] = (time, value) # this creates RunviewerClass for secondary board and calls get_traces for all channels
                            if False:
                                for dev in pseudoclock.child_list.values():
                                    print('adding Trigger %s board %s (class %s)' % (pseudoclock.name, dev.name, dev.device_class))
                                    traces[dev.name] = (time, value)
                elif self.type == TYPE_AO: # analog outputs (intermediate device)
                    save_print("'%s' add trace (analog out) %i samples" % (self.name, len(data)))
                    ...
                    save_print("'%s' add trace (digital out) %i samples" % (self.name, len(data)))
...
class FPGA_buttons(QWidget):
    ...
    def get_save_data(self, data):
        ...
        # update: only load data from connection table or default values
        pass
