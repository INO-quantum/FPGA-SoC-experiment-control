...
# labscript definition file for DDS AD9858
...
SYSCLK              = 1000e6        # TODO: to be checked!
...
class AD9858(DDS_generic):
    ...
    def init_hardware(properties):
        ...
        return AD9858.reset_hardware(properties)
    ...
    def reset_hardware(properties):
        ...
        return np.array([
                ...
                address|WRITE_AND_UPDATE|(0x58<<VALUE_SHIFT)|(0x00<<REG_SHIFT),     # 2 GHz divider disable; Mixer power down; Phase detect power down
                ], dtype=AD9858.dtype)
    ...
    def to_words(properties, value, update=True):
        ...
        if sub_channel == DDS_CHANNEL_FREQ:
            return AD9858.freq_to_words(properties, value, update)
        elif sub_channel == DDS_CHANNEL_AMP:
            return AD9858.amp_to_words(properties, value, update)
        elif sub_channel == DDS_CHANNEL_PHASE:
            return AD9858.phase_to_words(properties, value, update)
        else:
    ...
    def freq_to_words(properties, frequency, update=True):
        ...
        """
        address = (properties['address'] & ADDR_MASK) << ADDR_SHIFT
        f = np.round((np.clip(frequency, AD9858.freq_limits[0], AD9858.freq_limits[1])*(1<<FREQ_BITS))/SYSCLK).astype(AD9858.dtype) & FREQ_MASK
        f = [(f>>i) & VALUE_MASK for i in range(0, FREQ_BITS, VALUE_BITS)]
        write_last = WRITE_AND_UPDATE if update else WRITE
        return np.array([
                address|WRITE     |(f[0]<<VALUE_SHIFT)|(0x0a<<REG_SHIFT),
                address|WRITE     |(f[1]<<VALUE_SHIFT)|(0x0b<<REG_SHIFT),
                address|WRITE     |(f[2]<<VALUE_SHIFT)|(0x0c<<REG_SHIFT),
                address|write_last|(f[3]<<VALUE_SHIFT)|(0x0d<<REG_SHIFT)
                ], dtype=AD9858.dtype)
    ...
    def amp_to_words(properties, amplitude, update=True):
        ...
        """
        address = (properties['address'] & ADDR_MASK) << ADDR_SHIFT
        uval = 10**(np.clip(amplitude, AD9858.amp_limits[0], AD9858.amp_limits[1])/20)
        a = round(((uval - umin)*amax + (umax - uval)*amin)/(umax-umin)) & FREQ_MASK
        a = [(a>>i) & VALUE_MASK for i in range(0, AMP_BITS, VALUE_BITS)]
        write_last = WRITE_AND_UPDATE if update else WRITE
        return np.array([
                address|AMPLITUDE|(a[0]<<VALUE_SHIFT) # TODO: assumed this is also shifted?
                ], dtype=AD9858.dtype)
    ...
    def phase_to_words(properties, phase, update=True):
        ...
        write_last = WRITE_AND_UPDATE if update else WRITE
        return np.array([
                address|WRITE     |(p[0]<<VALUE_SHIFT)|(0x0e<<REG_SHIFT),
                address|write_last|(p[1]<<VALUE_SHIFT)|(0x0f<<REG_SHIFT)    # TODO: register 0x0f?
                ], dtype=AD9858.dtype)
