...
from labscript import PseudoclockDevice, Pseudoclock, ClockLine, IntermediateDevice, DDS, DigitalOut, AnalogOut, set_passed_properties, LabscriptError, config, Output, TriggerableDevice
...
IN_SRC_NONE     = 'None'
in_dests        = {'start trigger'  : (CTRL_IN_DST_START, CTRL_IN_SRC_IN_ALL, CTRL_IN_LVL_IN_ALL),
                   'stop trigger'   : (CTRL_IN_DST_STOP, CTRL_IN_SRC_ALL, CTRL_IN_LVL_ALL),
                   'restart trigger': (CTRL_IN_DST_RESTART, CTRL_IN_SRC_IN_ALL, CTRL_IN_LVL_IN_ALL),
                   'NOP bit'        : (CTRL_IN_DST_DATA_NOP, 1<<CTRL_IN_SRC_DATA_28, 1<<CTRL_IN_DATA_3),
                   'STRB bit'       : (CTRL_IN_DST_DATA_STRB, CTRL_IN_SRC_DATA_ALL, CTRL_IN_LVL_DATA_ALL),
                   'IRQ bit': (CTRL_IN_DST_DATA_IRQ, CTRL_IN_SRC_DATA_ALL, CTRL_IN_LVL_DATA_ALL)}
...
class FPGA_PseudoClock(Pseudoclock):
    ...
    def add_device(self, device):
        if isinstance(device, (ClockLine,TriggerableDevice)):
...
class FPGA_board(PseudoclockDevice):
    ...
    allowed_children = [FPGA_PseudoClock, TriggerableDevice]
    ...
    def __init__(self, name, ip_address, ip_port, bus_rate=MAX_BUS_RATE, num_racks=1, trigger_device=None, worker_args=None):
        if trigger_device is not None: 
            trigger_connection = 'trigger' # we have to give a connection. name seems irrelevant.
        else:                          
    ...
    def generate_code(self, hdf5_file):
        ...
        if generate_code_with_labscript:
            ...
            self.get_raw_data()
        print('total_time %.3fms\n' % ((get_ticks() - total_time) * 1e3))
        ...
        #update: take always first data
        chg[0] = True

        # update: take always last data
        chg[-1] = True
...
class FPGA_Worker(Worker):
    ...
    def transition_to_manual(self):
        ...
        # note: stop is resetting boards which also suspends output clock during reset.
