...
# works as PseudoClockDevice 03/07/2023
#####################################################################
...
worker_args_keys = [STR_CONFIG, STR_CONFIG_MANUAL, STR_INPUTS, STR_OUTPUTS, STR_EXT_CLOCK, STR_IGNORE_CLOCK_LOSS, STR_SYNC_WAIT, STR_SYNC_PHASE, STR_CYCLES, STR_SIMULATE]
...
in_dests        = {STR_TRIG_START       : (CTRL_IN_DST_START, CTRL_IN_SRC_IN_ALL, CTRL_IN_LVL_IN_ALL),
                   ...
                   STR_BIT_STRB         : (CTRL_IN_DST_DATA_IRQ, CTRL_IN_SRC_DATA_ALL, CTRL_IN_LVL_DATA_ALL)}
...
# do not change 'NOP bit' since it is used by driver on board to pad data. all data with this bit set is ignored by board.
...
def get_ctrl_in(input_selection):
    """
...
def get_in_selection(register, return_NONE=True, check=True):
    ...
    for dest, dest_offset in in_dests.items():
        reg_dest = (register >> dest_offset[0]) & CTRL_IN_DST_MASK;
    ...
    for key,value in worker_args.items():
        ...
        if (key == STR_CONFIG) or (key == STR_CONFIG_MANUAL):
            ...
            if not isinstance(value, bool):
                raise LabscriptError("worker_arg '%s':'%s' must be True/False (boolean)! but is %s" % (key, str(value), type(value)))
...
class FPGA_buttons(QWidget):
    def __init__(self, name, parent, update=[], worker_args={}):
        ...
        grid.addWidget(self.cb_allow_changes,1,0)
        ...
        grid.addWidget(self.cb_ext_clock,2,0)
        ...
        grid.addWidget(self.cb_ignore_clock_loss, 2, 1)
...
class FPGA_Tab(DeviceTab):
    ...
    def start_run(self, notify_queue):
        save_print('start run')
        # TODO: start all boards here. worker can use events to synchronize among them.
        #       NO: I start now also primary board directly in transition_to_buffered
        #           otherwise primary is also not started when there is for example PulseBlaster.
    ...
    def status_monitor(self, notify_queue):
        save_print('status monitor (FPGA)')
...
class FPGA_Worker(Worker):
    ...
    def transition_to_buffered(self, device_name, hdf5file, initial_values, fresh):
        ...
        if self.simulate:
            ...
            print('transition to buffered (simulate)')
            return {}
        ...
        with h5py.File(hdf5file,'r') as hdf5_file:
            ...
            if len(data) == 0:
                ...
                # update: start also primary board in order to have also PulseBlaster or other boards working simultaneously.
    ...
    def transition_to_manual(self):
        if self.simulate:
            print('transition to manual (simulate)')
            return True
    ...
    def status_monitor(self):
        ...
        if self.simulate:
            run_time = int((get_ticks() - self.t_start)*1e6)
            if run_time >= 3.0: return [True, self.get_warnings()]
            else: return [True, self.get_warnings()]
        if self.sock == None: # should never happen
            ...
            if result is None:
                ...
                if (cmd != SERVER_STATUS_RSP) and (cmd != SERVER_STATUS_IRQ_RSP):
                    ...
                    elif (self.board_status & STATUS_WAIT) or self.start_trg:
                        # TODO: update firmware such it sets WAIT bit, then we do not need to check for self.start_trg!
    ...
    def shutdown(self):
        if not self.simulate and self.sock is not None:
