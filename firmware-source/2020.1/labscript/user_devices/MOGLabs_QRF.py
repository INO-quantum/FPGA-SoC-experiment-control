...
# /MOGLabs_QRF.py                                                   #
#                                                                   #
# Copyright 2023, Florence University                               #
#                                                                   #
...
# timed table mode start trigger duration and delay. not measured
TABLE_TIMED_MIN_TRIGGER_DURATION = 1e-6
TABLE_TIMED_MIN_TRIGGER_DELAY    = 40e-9
TABLE_TIMED_MAX_TRIGGER_DELAY    = 40e-9

# triggered table mode trigger duration (not measured) and delay: jitter of 5us.
TABLE_MIN_TRIGGER_DURATION       = 1e-6
TABLE_MIN_TRIGGER_DELAY          = 3.5e-6
TABLE_MAX_TRIGGER_DELAY          = 8.5e-6

# no table mode trigger duration (not measured) and delay.
# maximum delay given from specs. minimum delay measured.
# no jitter seen but could be different from device to device.
NO_TABLE_MIN_TRIGGER_DURATION    = 1e-6
...
class QRF_DDS(IntermediateDevice):
    ...
    def __init__(self, name, parent_device, connection, digital_gate=None, table_mode=True, trigger_each_step=False, freq_limits=None, freq_conv_class=None, freq_conv_params={},
        ...
        if self.table_mode:
            if trigger_each_step:
                self.trigger_delay      = TABLE_MAX_TRIGGER_DELAY
                self.trigger_duration   = TABLE_MIN_TRIGGER_DURATION
            else:
                self.trigger_delay      = TABLE_TIMED_MAX_TRIGGER_DELAY
                self.trigger_duration   = TABLE_TIMED_MIN_TRIGGER_DURATION
            ...
            self.trigger_duration   = NO_TABLE_MIN_TRIGGER_DURATION
            self.bus_rate           = NO_TABLE_MAX_BUS_RATE
            self.clock_resolution   = 1e-6
            self.clock_limit        = get_clock_limit(self.clock_resolution)
        ...
        # notify MOGLabs_QRF of new pseudoclock. TODO: this will adapt the clock_limt and clock_resolution to fastest clock
...
class MOGLabs_QRF(PseudoclockDevice):
    ...
    trigger_delay = 0.0 # set to 0 since this is subtracted from all times which messes up my time checks.
    trigger_minimum_duration = 1e-6 # not sure?

    # Andi: added trigger_channel
    @set_passed_properties(
        property_names={'connection_table_properties': ['addr', 'port', 'worker_args']}
    )
    def __init__(self, name, parent_device, trigger_channel, addr=None, port=7802, worker_args=None):
        ...
        # parent device must be a DigitalChannels object
        # note: importing makes troubles, so have to check type instead of isinstance!
        if type(parent_device).__name__ != 'DigitalChannels':
            raise LabscriptError("device '%s' parent_device is '%s' but must be 'DigitalChannels'!" % (self.name, type(parent_device).__name__))

        # init parent class.
        # trigger_device  = DigitalChannels class object
        # trigger_connection = channel number (integer) of trigger_device
        # this will create a Trigger device used to trigger the QRF
        #print(f"parent device of QRF: '{parent_device}")
        PseudoclockDevice.__init__(self, name, trigger_device=parent_device, trigger_connection=trigger_channel)
    ...
    def generate_code(self, hdf5_file):
        ...
        # create list of times and raw_output for each device
            ...
            for clockline in pseudoclock.child_devices:
                ...
                for IM in clockline.child_devices:
                    ...
                    for dds in IM.child_devices:
                        ...
                        print("'%s' %i times:" % (dds.name, len(times)), times)
                        #print(f"'{self.name}' DDSs: {DDSs}")

                        # each DDS channel has a gate associated
                        # when table_mode = True and trigger_each_step = True then gate times must agree with times of channel
                        # the times are added at the beginning of FPGA_device.generate_code for all get_table_mode_channels.
                        gate = dds.gate
                        #parent = gate.parent_device
                        gate_cl = gate.parent_device.parent_device
                        gate_ps = gate_cl.parent_device
                        #print('DDS gate:', gate.name, "parent:", gate.parent_device.name, "channel:", gate.connection, "clockline:", gate_cl.name)
                        #print('DDS gate times:', gate_ps.times[gate_cl])
                        #print('DDS gate values:', gate.raw_output)
                        #print(gate.child_devices)

                        mask = (gate.raw_output == 1)
                        gate_active = gate_ps.times[gate_cl][mask]

                        if len(times) == 2: # no actions: gate must be 0 everywhere
                            if len(gate_active) != 0:
                                raise LabscriptError('%s error: no actions but gate has %i active times!' % (dds.name, len(gate_active)))
                        elif len(times) != len(gate_active) + 2:
                            print(times)
                            print(gate.raw_output)
                            print(gate_active)
                            raise LabscriptError('%s error: %i actions != %i + 2 gate active times!' % (dds.name, len(times), len(gate_active)))
                        elif np.any(gate_active != times[1:-1]):
                            print(gate_active)
                            print(times)
                            raise LabscriptError('%s error: gate times != times - trigger_delay %f!' % (dds.name, IM.trigger_delay))
                        ...
                        out_table['time'][:]  = (times/5e-6).astype(np.uint32) # time in units of 5us
