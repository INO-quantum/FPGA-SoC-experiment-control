...
# static_AO              = True/False = if True use only static analog channels.
# static_DO              = True/False = if True use only static digital channels.
...
# clock_rate             = external clock rate in Hz. Must be given in clock_terminal is not None.
...
# other options are possible. see NI_DAQmx_iPCdev/labscript_device.py __init__
NI_PXIe_6535_iPCdev(
    name                    = 'NI_board_0',
    MAX_name                = 'PXI1Slot2',
    parent_device           = None,
...
if False:
    ...
    ao_count = [0, 0]
    do_count = [0, 0]
    # number of static analog outputs to create for testing. not clear if some boards have only these?
    num_static_AO = 3
    # enable or disable static ports
    use_static_DO = True
