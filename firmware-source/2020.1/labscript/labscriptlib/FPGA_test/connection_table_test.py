...
secondary = FPGA_board(name='board1', ip_address='192.168.1.11', ip_port=DEFAULT_PORT, bus_rate=1.0, num_racks=1,
                       trigger_device=primary,       # for secondary board: give trigger_device = primary board.
                       worker_args={'ignore_clock_loss':False} # danger!
                      )
...
if False:
    ...
    AnalogOut(name='Cr_AOM_MOT_60to127MHz_no4_MHz', parent_device=AO1, connection='0x4',
              unit_conversion_class=generic_conversion,
              unit_conversion_parameters={'unit': 'MHz', 'equation': '(x-65.5)/17.1', 'min': 0.0,
                                          'max': 236.50000000000003})
    AnalogOut(name='Cr_AOM_TC_70to115MHz_no5_MHz', parent_device=AO1, connection='0x5',
              unit_conversion_class=generic_conversion,
    ...
    AnalogOut(name='test_ao_0', parent_device=AO0, connection='0x73')
    AnalogOut(name='test_ao_1', parent_device=AO0, connection='0x74')
    AnalogOut(name='test_ao_2', parent_device=AO0, connection='0x75')
    AnalogOut(name='test_ao_3', parent_device=AO0, connection='0x76')
    AnalogChannels(name='AO1', parent_device=board1, rack=0, max_channels=2)
    AnalogOut(name='test_ao_4', parent_device=AO1, connection='0x73')
    AnalogOut(name='test_ao_5', parent_device=AO1, connection='0x74')
...
DigitalChannels(name='DO2', parent_device=board1, connection='0x01', rack=0, max_channels = 16)
