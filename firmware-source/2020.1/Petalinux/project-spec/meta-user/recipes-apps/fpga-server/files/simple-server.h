...
#define SERVER_SEND_ABORT       (SERVER_ERROR + 0x5B)   // SendData could not send all data. sending was aborted.
...
// return values by onData, ONDATA_CLOSE_CLIENT can be combined with one of other return values to properly treat buffers before closing.
// any other value returned will be treated as ONDATA_CLOSE_CLIENT.
#define ONDATA_FREE_LAST        0x00010000          // last buffer can be freed (expect no more data)
#define ONDATA_FREE_ALL         0x00020000          // all buffers can be freed (expect no more data)
#define ONDATA_REUSE_LAST       0x00030000          // last buffer can be reused (expect more data)
#define ONDATA_REUSE_ALL        0x00040000          // all buffers can be reused (expect more data)
#define ONDATA_IN_USE_LAST      0x00050000          // last data is kept by onData (free from app)
#define ONDATA_IN_USE_ALL       0x00060000          // all data is kept by onData (free from app)
#define ONDATA_COLLECT_LAST     0x00070000          // collect last buffer of data (expect more data)
#define ONDATA_COLLECT_ALL      0x00080000          // collect all buffers of data (expect more data)
#define ONDATA_CLOSE_CLIENT     0x10000000          // close client (recommended on error)
#define ONDATA_MASK             (~ONDATA_CLOSE_CLIENT)  // mask for ONDATA_ return values excluding ONDATA_CLOSE_CLIENT

// data_info option for SendData/onSendFinished
#define ABORT_PARTIAL_DATA      0xffffffff          // SendData does aborts when partial data is sent. onSendFinished is not called.
#define DATA_CHAR_ARRAY         0xfffffffe          // data allocated with new char[#] which can be deleted with delete []
#define DATA_STATIC             0xfffffffd          // static data. do not delete. (note: never use data on stack!)
...
private:
    ...
    int bytes;
    int sent;
    unsigned data_info;                             // user-provided data info
public:
    send_data(void *data, int bytes, int bytes_sent, unsigned data_info);
    ~send_data();

    void *get_data(void) { return data; };                      // get data
    void *get_reset_data(void) { void *d = data; data = NULL; bytes = sent = 0; this->data_info=0; return d; }; // get and reset data.
    ...
    unsigned get_data_info(void) { return data_info; };         // get data info
...
class simple_server; // needed by send_delete_all
class client_info {
...
public:
    ...
    void send_delete_all(class simple_server *server);  // savely deletes send list
...
protected:
    ...
    // send data
    int SendData(client_info *client, void *data, int *num, unsigned data_info);    // send num bytes of data to client
...
public:
    // needs to be public
    virtual void onSendFinished(client_info *client, void *data, int num, int sent, unsigned data_info, int error) = 0;    // sending of large data finished
