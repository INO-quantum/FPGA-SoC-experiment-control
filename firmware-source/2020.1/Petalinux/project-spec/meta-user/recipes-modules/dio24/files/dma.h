...
#define DIO_REG_CTRL                ( REG_CTRL          *DIO_REG_BYTES )        // control register see DIO_CTRL_ definitions
#define DIO_REG_CTRL_IN             ( REG_CTRL_IN       *DIO_REG_BYTES )        // trigger control register see CTRL_TRG_ definitions
#define DIO_REG_CTRL_OUT            ( REG_CTRL_OUT      *DIO_REG_BYTES )        // output control register see CTRL_OUT_ definitions
#define DIO_REG_NUM_SAMPLES         ( REG_NUM_SAMPLES   *DIO_REG_BYTES )        // samples until stop, 0 = infinite
//#define DIO_REG_NUM_CYCLES          ( REG_NUM_CYCLES    *DIO_REG_BYTES )        // cycles until stop, 0 = infinite, 1 = default
#define DIO_REG_CLK_DIV             ( REG_CLK_DIV       *DIO_REG_BYTES )        // bus clock divider - 1. 8 bits.
#define DIO_REG_STRB_DELAY          ( REG_STRB_DELAY    *DIO_REG_BYTES )        // strobe delay {strb_end_1,strb_start_1,strb_end_0,strb_start_0} each 8 bits
#define DIO_REG_SYNC_DELAY          ( REG_SYNC_DELAY    *DIO_REG_BYTES )        // sync delay
#define DIO_REG_SYNC_PHASE          ( REG_SYNC_PHASE    *DIO_REG_BYTES )        // sync phase
// dio24 status registers
#define DIO_REG_STATUS              ( REG_STATUS        *DIO_REG_BYTES )        // status register, see STATUS_ definitions for bits
#define DIO_REG_BOARD_TIME          ( REG_BOARD_TIME    *DIO_REG_BYTES )        // actual board time
#define DIO_REG_BOARD_SAMPLES       ( REG_BOARD_SAMPLES *DIO_REG_BYTES )        // actual board samples
#define DIO_REG_BOARD_TIME_EXT      ( REG_BOARD_TIME_EXT*DIO_REG_BYTES )        // extra board time
#define DIO_REG_BOARD_SAMPLES_EXT   ( REG_BOARD_SAMPLES_EXT*DIO_REG_BYTES )     // extra board samples
//#define DIO_REG_BOARD_CYCLES        ( REG_BOARD_CYCLES  *DIO_REG_BYTES )        // number of cycles
#define DIO_REG_SYNC_TIME           ( REG_SYNC_TIME     *DIO_REG_BYTES )        // auto-sync round-trip time
#define DIO_REG_VERSION             ( REG_VERSION       *DIO_REG_BYTES )        // version
#define DIO_REG_INFO                ( REG_INFO          *DIO_REG_BYTES )        // info
