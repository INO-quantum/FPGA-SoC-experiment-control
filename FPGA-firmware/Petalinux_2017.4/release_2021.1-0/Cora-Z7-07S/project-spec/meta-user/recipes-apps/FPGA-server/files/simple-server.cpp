...
send_data::send_data(void *data, int bytes, int sent){
	next = NULL;
	this->data = data;
	this->bytes = bytes;
	this->sent = sent;
...
void client_info::send_delete_all(void) {
...
int simple_server::ReceiveData(client_info *&client) {
	...
	if(!err) {
		...
		else if (num > 0)
		{
			...
			switch(err & (~ONDATA_USER_BITS)) {
				...
				case ONDATA_COLLECT_ALL: // collect all buffers of data (no change here)
					break;
				case ONDATA_CLOSE_CLIENT: // close client (recommended on error)
					// same action as default
				default:
		...
		else
		{
			...
			// todo: maybe disconnect client here?
	...
	if (client == NULL) err = ERROR(0x0);
		...
		if (batch == NULL) err = ERROR(0x1);
			...
			if ((remaining == 0) || err) {
				...
				onSendFinished(client, batch->get_reset_data(), batch->get_bytes(), batch->get_sent(), err);
