...
// returned data of num bytes is saved in data and must be freed (except when NULL)
...
THREAD_ENTRY(FPGA_server::helper_thread, class FPGA_server *server)
{
    ...
    if(server == NULL) {
        ...
        sleep_ms(20);printf(HELPER "running ...\n");
                ...
                switch(active->cmd) {
                    case HELPER_CMD_WRITE:                  // write command
                        if ( active->data ) {
                            ...
                            //else { printf("helper %i: buffer %p bytes %u ok\n", wi->offset, wi->buffer, wi->bytes); sleep_ms(10); }
...
FPGA_server::FPGA_server(unsigned flags, const char *IP, const char *port, int num_cpu) {
    this->flags = flags;
...
// called every timeout ms. use for timing
...
inline int FPGA_server::collect_write_data(client_info *c, char *last_buffer, int last_bytes, int tot_bytes) {
    int err = ONDATA_CLOSE_CLIENT;
    ...
    if ((tot_bytes <= (RECV_BUFLEN/2)) && (bytes < this->b_set)) {
        err = ONDATA_COLLECT_LAST;                                                  // fill more data into buffer. too small buffers reduce DMA writing speed!
    }
    else {
        ...
        if ( helper_running ) {
            class queue_entry *q_entry;
            // note: we return ONDATA_IN_USE_ALL and server will free c->recv but not delete buffers used here.
            err = ONDATA_IN_USE_ALL;                                                // if no buffer is returned from helper: all buffers are used by helper
            while(next) {
            ...
            err = ONDATA_REUSE_ALL;                                                 // server can reuse list of buffers
            int b_req, b_written;
        ...
        if ( bytes >= this->b_set ) {                                               // all data received
            ...
            if (err > 0) {
#ifdef TIMING_TEST
                static uint32_t result[TIMING_TEST_NUM_COLS];
                ...
                result[0] = b_set/DIO_BYTES_PER_SAMPLE;                                 // total number of samples
                result[1] = b_first;                                                    // first bytes
                result[2] = t_RT;                                                       // RT time in cycles
                result[3] = t_upload;                                                   // upload time in cycles
                result[4] = t_tot;                                                      // total time in cycles
                result[5] = t_old;                                                      // total ticks in us
                result[6] = cpu_percent ? cpu_percent[0] : 0;                           // first CPU percent * 1000 (if no error)
                result[7] = (cpu_percent && (num_cpu > 1)) ? cpu_percent[1] : 0;        // second CPU percent * 1000 (if exists and no error)
                ...
                printf("%sOUT_WRITE CPU   (%%)    %u.%03u / %u.%03u\nappend result to %s\n\n", name, result[6]/1000, result[6]%1000, result[7]/1000, result[7]%1000, TIMING_TEST_FILE_NAME);
                save_CSV(TIMING_TEST_FILE_NAME, result, TIMING_TEST_NUM_COLS, TIMING_TEST_NUM_COLS);
                sleep_ms(100);
                if (cpu_percent) delete[] cpu_percent;
#else           // don't print during timing test!
...
#ifdef UPLOAD_AND_WRITE
                err |= ONDATA_ACK;                          // TODO: ONDATA_REUSE_ALL is at the moment keeping only last entry.   
#else
                ...
                if (err > 0) err = ONDATA_REUSE_ALL | ONDATA_ACK;
#endif
    ...
    //if ((err & (~ONDATA_USER_BITS)) == ONDATA_IN_USE_ALL) { printf("ONDATA_IN_USE_ALL\n");sleep_ms(10); }
    //if ((err & (~ONDATA_USER_BITS)) == ONDATA_REUSE_ALL) { printf("ONDATA_REUSE_ALL\n");sleep_ms(10); }
    //if ((err & ONDATA_USER_BITS) == ONDATA_ACK) { printf("ONDATA_ACK\n");sleep_ms(10); }
...
int FPGA_server::onData(client_info *c, char *last_buffer, int last_bytes, int tot_bytes) {
    int err = ONDATA_REUSE_LAST; // in normal case allow to reuse last_buffer
    SERVER_CMD cmd;
    //uint32_t ldata;
    ...
    if(active_cmd == SERVER_NONE) { // not collecting data
        if(tot_bytes < sizeof(SERVER_CMD)) { // first byte received = command
            ...
            err = ONDATA_CLOSE_CLIENT; // error if command not found: closes client
            for(int i = 0; i < SERVER_CMD_NUM; ++i) {
                if(cmd == (server_cmd[i] & 0xff)) {
                    active_cmd = server_cmd[i];
                    err = ONDATA_COLLECT_LAST; // want more data
                    break;
                }
            }
        }
    ...
    if(err & ONDATA_OK) { // all data received: act depending on command
        err = 0; // TODO: below I always |= but it should be = for first error!
        switch(cmd) {
                    ...
                    if(flags & FLAG_SERVER) {
                        ...
                        while(c) {
                            if(c->is_client()) {
                                ...
                                err = SendData(c, &shtd, &last_bytes);
                                if(err != 0) printf("%snotify %s:%s error %d.\n", name, c->get_IP_address(), c->get_port_str(), err);
                                else printf("%snotify %s:%s\n", name, c->get_IP_address(), c->get_port_str());
                            };
                            c = clients.get_next(c);
                        }
            ...
            case SERVER_RESET:
                ...
                if(dma24_dev == FILE_INVALID) err |= ONDATA_NACK;
                    ...
                    if (dma24_reset(dma24_dev)) err |= ONDATA_NACK;
                    else {
                        err |= ONDATA_ACK;
            ...
            case SERVER_CMD_OPEN_RESOURCE:            // connect (Win7/8 Visa driver)
                printf("%sOPEN\n", name);
                if((dma24_dev != FILE_INVALID) || (dio24_dev != FILE_INVALID)) err |= ONDATA_NACK;
                    ...
                    if(FILE_OPEN_ERROR(dma24_dev)) {
                        err |= ONDATA_NACK;
                        printf("%sOPEN dma24_dev open failed!\n", name);
                    }
                    else {
                        dio24_dev = dio24_open(0);
                        if(FILE_OPEN_ERROR(dio24_dev)) {
                            printf("%sOPEN dio24_dev open failed!\n", name);
                            err |= ONDATA_NACK;
            ...
            case SERVER_GET_FPGA_STATUS_BITS:
                ...
                if(dma24_dev == FILE_INVALID) err |= ONDATA_NACK;
                    ...
                    if(cd == NULL) err |= ONDATA_NACK;
                        ...
                        err = SendData(c, (char*)cd, &last_bytes);
                        if(err) ONDATA_REUSE_LAST | ONDATA_NACK;
                        else err = ONDATA_REUSE_LAST;
                        delete cd;
                    }
                }
                break;
                ...
                if(dio24_dev == FILE_INVALID) err |= ONDATA_NACK;
                    ...
                    if(status == NULL) err |= ONDATA_NACK;
                        ...
                        if(err) {
                            ...
                            err = ONDATA_REUSE_LAST | ONDATA_NACK;
                        }                        
                        else {
                        ...
                        // updates with guaranteed rate but blocks until irq or timeout (1s)
                        err = read(dio24_dev, &status->status, sizeof(struct FPGA_status_run));
                        if(err != sizeof(struct FPGA_status_run)) {
                            ...
                            err = SendData(c, status, &last_bytes);
                            if(err) {
                                                printf("%sGET_STATUS send %d/%d bytes error %d\n", name, last_bytes, sizeof(struct client_status), err);
                                err = ONDATA_CLOSE_CLIENT;
                            }
                            else {
                                err = ONDATA_REUSE_LAST;
                ...
                if(dio24_dev == FILE_INVALID) err |= ONDATA_NACK;
                    ...
                    if(status == NULL) err |= ONDATA_NACK;
                        ...
                        if(err) {
                            ...
                            err = SendData(c, status, &last_bytes);
                            if(err) {
                                printf("%sGET_STATUS_FULL send %d/%d bytes error %d\n", name, last_bytes, sizeof(struct client_status_full), err);
                                err = ONDATA_CLOSE_CLIENT;
                            }
                            else {
                                err = ONDATA_REUSE_LAST;
                                ...
                                if((t_act - t_old) > 1000000) {
                                    t_old = t_act;
                                    printf("%sGET_STATUS_FULL 0x%8x %u us # %u\n", name, status->status.status_FPGA.status, status->status.status_FPGA.board_time, status->status.status_FPGA.board_samples);
                                }
                            }
                        }
                        delete status;
...
/*                        // set external clock frequency if should be used
                        uint32_t clock_Hz;
                        if( config->config & DIO_CTRL_EXT_CLK ) {
                            clock_Hz = config->clock_Hz;
                            err = dio24_set_ext_clock(dma24_dev, &clock_Hz); // set clock, return new clock frequency
#ifdef NO_HARDWARE
                            clock_Hz = 10000000;
#endif
                        }
                        else {
                            err = dio24_get_int_clock(dma24_dev, &clock_Hz); // get internal clock frequency
#ifdef NO_HARDWARE
                            clock_Hz = 100000000;
#endif
                        }
                        if(err) err = ONDATA_REUSE_LAST | ONDATA_NACK;
                        else {
                            // set multiplicator to closest integer
                            uint32_t mult = clock_Hz / config->scan_Hz;
                            if(mult < 1) mult = 1; // smallest allowed multiplicator is 1
                            dio24_set_mult(dma24_dev, &mult);
*/
                            /* set external/internal clock if has changed (implemented in set_config)
                            if ( (conf->config & DIO_CTRL_EXT_CLK) != (status.FPGA_ctrl & DIO_CTRL_EXT_CLK) ) {
                                err = dio24_set_clock_source(dev, (conf->config & DIO_CTRL_EXT_CLK) ? 1 : 0);
                                if(err) printf("out_config: set clock source = %s. error %d!\n", (conf->config & DIO_CTRL_EXT_CLK) ? "ext" : "int", err);
                                else printf("out_config: set clock source = %s. ok\n", (conf->config & DIO_CTRL_EXT_CLK) ? "ext" : "int");
                            }*/
                            ...
                            //printf("%sOUT_CONFIG actual config %x (%d)\n", name, config->config,err);sleep_ms(100);
                            //if(!err) err = dio24_set_extrig(dma24_dev, &config->extrig);
                            if(err) err = ONDATA_REUSE_LAST | ONDATA_NACK;
                            else {
                                ...
                                err = SendData(c, last_buffer, &last_bytes);     
                                if(err) err = ONDATA_CLOSE_CLIENT;
                                else err = ONDATA_REUSE_LAST;
                            }
                        //}
                    }
                ...
                if(dma24_dev == FILE_INVALID) err |= ONDATA_NACK;
...
#endif
                    {
                        ...
                        if(err < 0) {
                            ...
                            //printf("%sOUT_START reps = %d ok\n", name, cd->data);
                            err = ONDATA_REUSE_LAST | ONDATA_ACK;
                            t_old = get_ticks();
                        }
                    }
                }
            ...
            case SERVER_CMD_OUT_STOP:
                ...
                break;
            case SERVER_TEST:
                //printf("%sSERVER_TEST (unmounting SD)", name);
                ...
                if(dma24_dev == FILE_INVALID) err = ONDATA_NACK;
                    ...
                    if(err != (cd64->data_0 & SYNC_DELAY_MASK)) err = ONDATA_NACK;
                        ...
                        //else 
                        {
                            ...
                            while(dio24_get_status_FPGA(dma24_dev) & DIO_STATUS_PS_ACTIVE) { if(++err >=10) break; sleep_ms(10); }
                            if (err == 1) { printf("%snote: waited 1 loop for phase shift (ok)\n", name); sleep_ms(10); }
                            if (err > 1) { printf("%serror: waited %d loops for phase shift\n", name, err); err = ONDATA_NACK; }
                            else {
                                ...
                                if (cmd == SERVER_AUTO_SYNC_START) {
                                    ...
                                    t_sync = (cd64->data_0 & SYNC_DELAY_WITH_FET) ? AUTO_SYNC_SEC_CONF_FET : AUTO_SYNC_SEC_CONF;
...
#endif
                                    printf("%sAUTO-SYNC (STOP) phase %u status 0x%08x time 0x%08x ACK\n", name, act_phase, dio24_get_status_FPGA(dma24_dev), dio24_get_sync_time(dma24_dev));sleep_ms(10);
                                    act_phase = (int)(cd64->data_1 & SYNC_PHASE_MASK_1);
                                    // send back sync_time
                                    cd64->data_0 = t_sync;
                                    cd64->data_1 = 0;
                                    SendData(c, last_buffer, &last_bytes);
                                    err = ONDATA_REUSE_LAST; // no ACK is needed. we already returned same command.
                                }
                            }
                        }
                    }
            ...
            case SERVER_CMD_SET_ATTRIBUTE:
                ...
                err = ONDATA_CLOSE_CLIENT | ONDATA_NACK;
                break;
            default:
    ...
    if(err & ONDATA_ACK) { // send ACK
        //printf("%sACK\n", name);
        last_bytes = sizeof(SERVER_CMD);
        SendData(c, &ack, &last_bytes);
    }
    else if(err & ONDATA_NACK) { // error = send NACK
        printf("%sNACK (0x%d)\n", name, err);
        last_bytes = sizeof(SERVER_CMD);
        SendData(c, &nack, &last_bytes);
        // TODO: always close client after sending NACK
        //       otherwise client disconnects but server does not recognize?
        err |= ONDATA_CLOSE_CLIENT;
    }
    // on serious error close client and dma24/dio24_dev
    if ((err & (~ONDATA_USER_BITS)) == ONDATA_CLOSE_CLIENT) {
...
void FPGA_server::onSendFinished(client_info *c, void *data, int num, int sent, int error) {
    printf("%ssending of %d/%d bytes finished with error 0x%04X\n", name, sent, num, error);
}
...
int main(int argc, char **argv)
{
    ...
    int opt, num, num_cpu = 2;      // options, counter
    ...
    while ((opt = getopt(argc, argv, ":a:p:sqw:r:t:n:")) != -1) {
        switch (opt) {
            case 'a': address = optarg; break; // remote server IP address
            case 'p': port = optarg; break; // server port (remote or local)
...
//        printf("    -r file = read file\n");
    ...
    if(!err)
    {
        ...
        server = new FPGA_server(flags, (flags & FLAG_SERVER) ? SERVER_LOCAL_IP : (address ? address : SERVER_GLOBAL_IP), port, num_cpu);
        if (server == NULL) {
            printf("%sinit failed\n", name);
...
//sleep(10);     
#ifdef _WINDOWS
        // clean up windows sockets (i.e. free resources and terminate WS2_32.dll)
        printf("%sunload dll\n", name);
        WSACleanup();
#endif
    }
