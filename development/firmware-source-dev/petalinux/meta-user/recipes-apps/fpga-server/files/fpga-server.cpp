    ...
    if (sock < 0) {
        ...
        if ( state < 0 ) {
            ...
            if ((flags & IFF_UP) == 0) { //|| (!(flags & IFF_RUNNING)) ) {
                ...
                // get actual IP address (fails with errno 99 when kernel configured for DHCP)
                state = ioctl(sock, SIOCGIFADDR, &ifr);
                if ( state < 0 ) {
                    printf("get IP (1) failed. flags = 0x%x, errno = %d (ok with DHCP)\n", flags, errno);
                }
                else {
...
// TODO: this function should be moved entirely into to driver!
//       i.e. DIO24_IOCTL_SET_SYNC_PHASE
//       this should be merged into one function which locks to the external clock
//       and then does the phase shift.
//       uses client_data64 with data0 = nonzero to use external clock and 0 to unlock.
int set_sync_phase(int dma24_dev, uint32_t phase, char* name) {
    int err, count = 0;

    /* set phase shift. this starts phase shift if not the same as before. returns difference to old phase.
    dio24_set_sync_phase(dma24_dev, phase & SYNC_PHASE_MASK_2);
    if(err) {
        printf("%sset sync_phase 0x%x failed with error %d!\n", name, cd->data & SYNC_PHASE_MASK_2, err);
        err = ONDATA_NACK;
    }
    else {
    ...
    // check if external clock is locked:
    // if locked: set phase shift even with phase = 0
    // if not locked: if phase != 0: return error
    //                else         : return ok
    //err = dio24_get_status_FPGA(dma24_dev);
    struct set_reg32 sr32 = { .reg = DIO_REG_STATUS, .data = 0 };
    err = ioctl(dma24_dev, DIO24_IOCTL_GET_REG, &sr32);
    if (err) {
        printf("%sset_sync_phase reading status error %d (%d)\n", name, errno, err);
        err = ONDATA_NACK;
    }
    else if ((sr32.data & DIO_STATUS_EXT_LOCKED) == 0) {
        if (phase != 0) {
            printf("%sset_sync_phase ext/det %u/%u no external clock! (status 0x%x)\n", name, 
                (unsigned)((phase >> SYNC_PHASE_BITS) & SYNC_PHASE_MASK_1),
                (unsigned)(phase & SYNC_PHASE_MASK_1),
                (unsigned)sr32.data);
            err = ONDATA_NACK;
        }
        else {
        ...
        // do phase shift and wait until its done
        // TODO: if phase shift is nonzero wait for PS_ACTIVE bit set and then reset, otherwise we might miss it?
        //err = dio24_set_sync_phase(dma24_dev, phase);
        sr32.reg  = DIO_REG_SYNC_PHASE;
        sr32.data = phase;
        err = ioctl(dma24_dev, DIO24_IOCTL_GET_REG, &sr32);
        if(err) {
            printf("%sset_sync_phase 0x%x failed with error %d (%d)!\n", name, phase, errno, err);
            err = ONDATA_NACK;
        }
        else {
            // wait until phase shift is done
            //while(dio24_get_status_FPGA(dma24_dev) & DIO_STATUS_PS_ACTIVE) { 
            sr32.reg = DIO_REG_STATUS;
            while (true) {
                err = ioctl(dma24_dev, DIO24_IOCTL_GET_REG, &sr32);
                if ( err || ((sr32.data & DIO_STATUS_PS_ACTIVE) == 0) || (++count >= SERVER_PHASE_RETRY) ) break;
                sleep_ms(1); // TODO: maybe sleep only 100us?
            }
            if (count >= SERVER_PHASE_RETRY) { 
                printf("%sset_sync_phase ext/det %u/%u error! waited %d loops, status 0x%x\n", name, 
                    (unsigned)((phase >> SYNC_PHASE_BITS) & SYNC_PHASE_MASK_1),
                    (unsigned)(phase & SYNC_PHASE_MASK_1),
                    count,
                    sr32.data);
                ...
                //printf("%snote: waited %i loop(s) for phase shift (ok)\n", name, err);
                printf("%sset_sync_phase ext/det %u/%u (%d loops) ok\n", name, 
                    (unsigned)((phase >> SYNC_PHASE_BITS) & SYNC_PHASE_MASK_1),
                    (unsigned)(phase & SYNC_PHASE_MASK_1),
                    count);
                err = ONDATA_ACK;
...
int config_board(int dma24_dev, char *name, uint32_t clk_div, uint32_t strb_delay, uint32_t sync_wait, uint32_t sync_phase, 
            uint32_t ctrl, uint32_t ctrl_in[2], uint32_t ctrl_out[2]) {
    int err = 0;
    struct set_reg32 sr32;
    
    // set clock divider
    if ((clk_div < CLK_DIV_MIN) || (clk_div > CLK_DIV_MAX)) {
        printf("%sset_div %u outside range [%d,%d]!\n", name, clk_div, CLK_DIV_MIN, CLK_DIV_MAX);
        err = ONDATA_NACK;
    }
    else {
        //dio24_set_div(dma24_dev, value, sr32, err);
        sr32.reg  = DIO_REG_CLK_DIV;
        sr32.data = clk_div;
        err = ioctl(dma24_dev, DIO24_IOCTL_SET_REG, &sr32);
        if(err) {
            printf("%sset_div %u failed with error %d (%d)!\n", name, sr32.data, errno, err);
            err = ONDATA_NACK;
        }
        else {
            ...
            sr32.data = strb_delay;
            //dio24_set_strb_delay(dma24_dev, value, sr32, err);
            err = ioctl(dma24_dev, DIO24_IOCTL_SET_REG, &sr32);
            if(err) {
                printf("%sset strobe delay 0x%x failed with error %d (%d)!\n", name, sr32.data, errno, err);
                err = ONDATA_NACK;
            }
            else {
                printf("%sset strobe delay 0x%x ok\n", name, sr32.data);

                // set board waiting time
                sr32.reg  = DIO_REG_SYNC_DELAY;
                sr32.data = sync_wait;
                //dio24_set_sync_delay(dma24_dev, value, sr32, err);
                err = ioctl(dma24_dev, DIO24_IOCTL_SET_REG, &sr32);
                if (err != 0) { 
                    printf("%sset wait time %u failed with error %d (%d)!\n", name, sr32.data, errno, err);
                    err = ONDATA_NACK;
                }
                else {
                    ...
                    // set sync phase
                    // note: this requires external clock locked!
                    // TODO: this has to be changed and done only after locking. maybe just by start.
                    sr32.data = sync_phase;
                    if (sr32.data != 0) err = set_sync_phase(dma24_dev, sr32.data, name);
                    else                err = ONDATA_ACK;
                    if (err == ONDATA_ACK) {
                        // set configuration bits
                        // TODO: the 5 registers could be set in one command here?
                        //printf("%sOUT_CONFIG set config %x\n", name, config->config); //sleep_ms(100);
                        //dio24_set_config(dma24_dev, config->config, sr32, err);
                        sr32.reg  = DIO_REG_CTRL;
                        sr32.data = ctrl;
                        err = ioctl(dma24_dev, DIO24_IOCTL_SET_REG, &sr32);
                        printf("%sOUT_CONFIG actual config %x (%d)\n", name, ctrl, err); //sleep_ms(20);
                        if(!err) {
                            // set input control register 0
                            //dio24_set_ctrl_in0(dma24_dev, config->ctrl_in0, sr32, err);
                            sr32.reg  = DIO_REG_CTRL_IN0;
                            sr32.data = ctrl_in[0];
                            err = ioctl(dma24_dev, DIO24_IOCTL_SET_REG, &sr32);
                            if(err) {
                                printf("%sset ctrl_in0 0x%x != 0x%x\n", name, ctrl_in[0], err); //sleep_ms(10);
                                err = ONDATA_NACK;
                            }
                            else {
                                // set input control register 1
                                //dio24_set_ctrl_in1(dma24_dev, config->ctrl_in1, sr32, err);
                                sr32.reg  = DIO_REG_CTRL_IN1;
                                sr32.data = ctrl_in[1];
                                err = ioctl(dma24_dev, DIO24_IOCTL_SET_REG, &sr32);
                                if(err != 0) {
                                    printf("%sset ctrl_in1 0x%x != 0x%x\n", name, ctrl_in[1], err); //sleep_ms(10);
                                    err = ONDATA_NACK;
                                }
                                else {
                                    //printf("%sset in ctrl 0x%x ok\n", name, config->ctrl_in); //sleep_ms(10);
                                    // set output control register 0
                                    //dio24_set_ctrl_out0(dma24_dev, config->ctrl_out0, sr32, err);
                                    sr32.reg  = DIO_REG_CTRL_OUT0;
                                    sr32.data = ctrl_out[0];
                                    err = ioctl(dma24_dev, DIO24_IOCTL_SET_REG, &sr32);
                                    if(err != 0) {
                                        printf("%sset ctrl_out0 0x%x != 0x%x\n", name, ctrl_out[0], err); //sleep_ms(10);
                                        err = ONDATA_NACK;
                                    }
                                    else {
                                        // set output control register 1
                                        //dio24_set_ctrl_out1(dma24_dev, config->ctrl_out1, sr32, err);
                                        sr32.reg  = DIO_REG_CTRL_OUT1;
                                        sr32.data = ctrl_out[1];
                                        err = ioctl(dma24_dev, DIO24_IOCTL_SET_REG, &sr32);
                                        if(err != 0) {
                                            printf("%sset ctrl_out1 0x%x != 0x%x\n", name, ctrl_out[1], err); //sleep_ms(10);
                                            err = ONDATA_NACK;
                                        }
                                        else {
                                            printf("%sset ctrl_in/out 0x%x/%x/%x/%x (ok)\n", name, ctrl_in[0], ctrl_in[1], ctrl_out[0], ctrl_out[1]); //sleep_ms(10);
                                        }
                                    }
                                }
                            }
...
FPGA_server::FPGA_server(unsigned flags, const char *IP, const char *port, 
    ...
    this->clk_div = clk_div;
    this->ctrl_in[0] = ctrl_in[0];
    this->ctrl_in[1] = ctrl_in[1];
    this->ctrl_out[0] = ctrl_out[0];
    this->ctrl_out[1] = ctrl_out[1];
            ...
            if (err) printf("%shelper thread startup failed with error %d\n", name, err);
                ...
                if(FILE_OPEN_ERROR(dma24_dev)) {
                    ...
                    if(FILE_OPEN_ERROR(dio24_dev)) {
                        ...
                        err = config_board(dma24_dev, this->name,
                            this->clk_div,      // clk_div 
                            this->strb_delay,   // strb_delay 
                            this->sync_wait,    // sync_wait
                            this->sync_phase,   // sync_phase 
                            this->primary ? SERVER_CONFIG_DEFAULT_PRIM : SERVER_CONFIG_DEFAULT_SEC, // FPGA control register 
                            this->ctrl_in,      // input control register 
                            this->ctrl_out      // output control register
                            );  
...
int FPGA_server::onData(client_info *c, char *last_buffer, int last_bytes, int tot_bytes) {
        ...
        switch(cmd) {
            ...
            case SERVER_RESET:
                if(dma24_dev == FILE_INVALID) {
                    ...
                    if (ioctl(dma24_dev, DMA24_IOCTL_RESET, NULL)) {
                        printf("%sRESET NACK\n", name);
                        err = ONDATA_NACK;
                    }
                    else {
            ...
            case SERVER_CMD_CLOSE:                // disconnect
                ...
                if (dma24_dev != FILE_INVALID) close(dma24_dev);
                //if (dio24_dev != FILE_INVALID) dio24_close(dio24_dev);
                if (dio24_dev != FILE_INVALID) close(dio24_dev);
            ...
            case SERVER_GET_FPGA_STATUS_BITS:
                ...
                if(dma24_dev == FILE_INVALID) err = ONDATA_NACK;
                    ...
                    if(cd == NULL) err = ONDATA_NACK;
                        ...
                        struct set_reg32 sr32 = { .reg = DIO_REG_STATUS, .data = 0 };
                        //err = dio24_get_status_FPGA(dma24_dev, sr32, cd->data);
                        err = ioctl(dma24_dev, DIO24_IOCTL_GET_REG, &sr32);
                        if (err) {
                            err = ONDATA_NACK;
                            delete cd;
            ...
            case SERVER_GET_DMA_STATUS_BITS:
                ...
                if(dma24_dev == FILE_INVALID) err = ONDATA_NACK;
                    ...
                    if(cd == NULL) err = ONDATA_NACK;
                        ...
                        //cd->data = dma24_get_config(dma24_dev); // TODO: this might return -1 for valid config!
                        cd->data = ioctl(dma24_dev, DMA24_IOCTL_GET_CONFIG, 0);
                        last_bytes = sizeof(struct client_data32);
            ...
            case SERVER_GET_STATUS_IRQ:
                ...
                if(dio24_dev == FILE_INVALID) err = ONDATA_NACK;
                    ...
                    if(status == NULL) err = ONDATA_NACK;
                        ...
                        err = ioctl(dma24_dev, DIO24_IOCTL_GET_STATUS_RUN, &status->status);
                        if(err) {
                            printf("%sGET_STATUS error %d (%d)\n", name, errno, err);
                            err = ONDATA_NACK;
                            delete status;
                        }                        
                        else {
                        ...
                        if(err == sizeof(struct FPGA_status_run)) err = 0;
                            ...
                            // timeout: try to read status directly not waiting for IRQ
                            status->cmd = SERVER_RSP_STATUS;
                            //err = dio24_get_status_run(dma24_dev, &status->status);
                            err = ioctl(dma24_dev, DIO24_IOCTL_GET_STATUS_RUN, &status->status);
                        }
                        if(err) {
                            if (status->cmd == SERVER_RSP_STATUS_IRQ) printf("%sGET_STATUS_IRQ error %d (%d)\n", name, errno, err);
                            else                                      printf("%sGET_STATUS (no IRQ) error %d (%d)\n", name, errno, err);
                            err = ONDATA_NACK;
                            delete status;
                        }
                        else {
            ...
            case SERVER_GET_STATUS_FULL: 
                ...
                if(dio24_dev == FILE_INVALID) err = ONDATA_NACK;
                    ...
                    if(status == NULL) err = ONDATA_NACK;
                        ...
                        status->status.ctrl_FPGA = FPGA_STATUS_SHOW; // show status in driver
                        //err = dio24_get_status(dma24_dev, &status->status);
                        err = ioctl(dma24_dev, DIO24_IOCTL_GET_STATUS, &status->status);
                        if(err) {
                            printf("%sGET_STATUS_FULL error %d (%d)\n", name, errno, err);
                            err = ONDATA_NACK;
                            delete status;
                        }
                        else {
            ...
            /*case SERVER_GET_INFO:
                if(dma24_dev == FILE_INVALID) err = ONDATA_NACK;
                    ...
                    if (cd64 == NULL) err = ONDATA_NACK;
                        ...
                        cd64->data_0 = dio24_get_info_  (dma24_dev);
                        cd64->data_1 = dio24_get_version(dma24_dev);
            ...
            case SERVER_CMD_OUT_CONFIG:
                ...
                if(dma24_dev == FILE_INVALID) err = ONDATA_NACK;
                    ...
                    if((config->scan_Hz == 0) || ((config->config & DIO_CTRL_EXT_CLK) && (config->clock_Hz == 0))) err = ONDATA_NACK;
                        ...
                        // configure board
                        err = config_board(dma24_dev, this->name, 
                            (uint32_t)(config->clock_Hz/config->scan_Hz),   // clk_div 
                            (config->strb_delay  == STRB_DELAY_AUTO) ? this->strb_delay : config->strb_delay, // strb_delay, 
                            (config->sync_wait   == SYNC_DELAY_AUTO) ? this->sync_wait  : config->sync_wait, //sync_wait, 
                            ((config->sync_phase == SYNC_PHASE_AUTO) ? this->sync_phase : config->sync_phase) & SYNC_PHASE_MASK_2, //sync_phase, 
                            config->config,     // FPGA control register 
                            config->ctrl_in,    // input control register 
                            config->ctrl_out    // output control register
                            );
            ...
            case SERVER_CMD_OUT_START:
                ...
                if(dma24_dev == FILE_INVALID) {
...
#endif
                    {
                        ...
                        if(err < 0) {
                            printf("%sOUT_START cycles = %d error %d (%d)\n", name, cd->data, errno, err); //sleep_ms(100);
                            err = ONDATA_NACK;
                        }
                        else {
                            printf("%sOUT_START cycles = %d ok (%uus)\n", name, cd->data, get_ticks()-t_start); //sleep_ms(100);
                            err = ONDATA_ACK;
                            t_old = get_ticks();
                        }
                    }
            ...
            case SERVER_CMD_OUT_STOP:
                ...
                if(dma24_dev == FILE_INVALID) err = ONDATA_NACK;
                    ...
                    err = ioctl(dma24_dev, DMA24_IOCTL_STOP, 0);
                    if(err < 0) {
                ...
                break;
            case SERVER_SET_EXT_CLOCK:
                // set external clock phase and wait until done
                // last_buffer = client_data32 with .data = sync_phase
                // if external clock not locked and phase != 0 will return ONDATA_NACK
                // on success returns ONDATA_ACK
                if(dma24_dev == FILE_INVALID) err = ONDATA_NACK;
                else {
                    struct client_data32 *cd = (struct client_data32 *)last_buffer;
                    err = set_sync_phase(dma24_dev, cd->data, name);
                }
                break;
            case SERVER_GET_REG:
            case SERVER_SET_REG:
                if ((dma24_dev == FILE_INVALID) || (last_bytes != sizeof(struct client_sr32))) err = ONDATA_NACK;
                else {
                    struct client_sr32 *sr32 = (struct client_sr32 *)last_buffer;
                    if (cmd == SERVER_SET_REG) {
                        //err = dio24_set_reg(dma24_dev, &sr32->sr32);
                        err = ioctl(dma24_dev, DIO24_IOCTL_SET_REG, &sr32->sr32);
                    }
                    else {
                        //sr32->sr32.data = dio24_get_reg(dma24_dev, sr32->sr32.reg);
                        err = ioctl(dma24_dev, DIO24_IOCTL_GET_REG, &sr32->sr32);
                    }
                    if (err != 0) {
                        printf("%sSET/GET_REG 0x%x, data 0x%x, error %d (%d)\n", name, sr32->sr32.reg, sr32->sr32.data, errno, err);
                        err = ONDATA_NACK;
                    }
                    else {
                        err = SendData(c, last_buffer, &last_bytes, DATA_CHAR_ARRAY);
                        if (err) {
                            if (err == SERVER_SEND_PENDING) { // onSendFinished called when finished (will delete buffer)
                                result = ONDATA_IN_USE_LAST;
                                printf("%sSET/GET_REG send %u bytes postboned (ok)\n", name, sizeof(struct set_reg32));
                            }
                            else {
                                result = ONDATA_CLOSE_CLIENT; // error: sending of NACK will most likely also fail. just close client.
                                printf("%sSET/GET_REG send %u bytes error %d\n", name, sizeof(struct set_reg32), err);
                            }
                        }
                        else {
                            err = ONDATA_NONE; // no ACK is needed. we already returned same command.
                            printf("%sSET/GET_REG 0x%x, data 0x%x ok\n", name, sr32->sr32.reg, sr32->sr32.data);
                        }
                    }
                }
                break;
    ...
    if(err == ONDATA_ACK) { // send ACK
        ...
        printf("%sNACK (0x%d)\n", name, err);
        last_bytes = sizeof(SERVER_CMD);
        err = SendData(c, &nack, &last_bytes, ABORT_PARTIAL_DATA);
        if (err) {
            result |= ONDATA_CLOSE_CLIENT; // error: close client but keep other return flags.
    ...
    if ((err < 0) || (result & ONDATA_CLOSE_CLIENT)) {
        ...
        if (this->dio24_dev != FILE_INVALID) close(this->dio24_dev);
        //if (this->dma24_dev != FILE_INVALID) dma24_close(this->dma24_dev);
        if (this->dma24_dev != FILE_INVALID) close(this->dma24_dev);
...
int main(int argc, char **argv)
{
    ...
    struct client_config c_config {
        ...
        .scan_Hz   = BUS_CLOCK_FREQ_HZ/CLK_DIV_DEFAULT,
        .config    = SERVER_CONFIG_DEFAULT_PRIM,
    ...
    int opt, i, num;
    int num_cpu = SERVER_CPUS_DEFAULT;          // number of CPUs
    ...
    uint32_t ctrl_in[2] = CTRL_IN_DEFAULT, ctrl_out[2] = CTRL_OUT_DEFAULT, clk_div = CLK_DIV_DEFAULT;
    int strb_cnt = 0;
    bool primary = true;            // primary board (true) or secondary board (false)
    ...
    while ((opt = getopt(argc, argv, ":sqI:P:d:i:o:b:p:c:w:f:")) != -1) {
        switch (opt) {
            case 's': 
                flags |= FLAG_SERVER; 
                break;
            ...
            case 'd': 
                clk_div = atoi(optarg); 
                if ((clk_div < CLK_DIV_MIN) || (clk_div > CLK_DIV_MAX)) err = -2; 
                else {
                    c_config.scan_Hz = c_config.clock_Hz/clk_div; // update for strobe calculation
                }
                break;
            case 'i': {
                    unsigned bytes   = 0;
                    unsigned entries = 2;
                    unsigned next    = 0;
                    err = read_list(optarg, &bytes, ctrl_in, &entries, &next, "[", ",", "]", "");
                    if ((err == 0) && (entries == 2)) printf("%sctrl_in = [0x%x,0x%x]\n", name, ctrl_in[0], ctrl_in[1]); 
                    else                              printf("%serror %d reading ctrl_in '%s'\n", name, err, optarg);
                }
                break;
            case 'o': {
                    unsigned bytes   = 0;
                    unsigned entries = 2;
                    unsigned next    = 0;
                    err = read_list(optarg, &bytes, ctrl_out, &entries, &next, "[", ",", "]", "");
                    if ((err == 0) && (entries == 2)) printf("%sctrl_out = [0x%x,0x%x]\n", name, ctrl_out[0], ctrl_out[1]); 
                    else                              printf("%serror %d reading ctrl_out '%s'\n", name, err, optarg);
                }
                break;
            case 'b': 
                if (strb_cnt < MAX_NUM_RACKS) strb_str[strb_cnt++] = optarg; 
                else err = -2; 
                break;
            case 'p': 
                primary = (atoi(optarg) == 1); 
                break;
            case 'c': 
                num_cpu = atoi(optarg); 
                if ((num_cpu <= 0)||(num_cpu > 2)) err = -2; 
                break;
            case 'w': 
                sync_wait = atoi(optarg); 
                if (sync_wait < 0) err = -2; 
                break;
            case 'f': 
                if(sscanf(optarg, "0x%x", &sync_phase) != 1) err = -7; 
                break;
            ...
            case '?': 
                printf("%sunknown option\n", name); 
                err = -2; 
                break;
            case ':': 
                printf("%sgive a value!\n", name); 
                err = -2; 
                break;
            default: 
                err = -3;
    ...
    if (err) {
        printf("error %d, given command line arguments:\n", err);
        for (i = 0; argv[i] != NULL; ++i) printf("%s\n", argv[i]);
        printf("\nchoose option:\n");
        printf("%s [-s] [-q] [-I <IP>] [-P #] [-d #] [-i [#,#]] [-o [#,#]] [-b <strb>] [-p <1/2>] [-c <1/2>] [-w #] [-f #]\n", argv[0]);
        printf("-s        : start server\n");
        printf("-q        : quit server (localhost only)\n");
        printf("-I <IP>   : server static IP address. default dynamic (DHCP)\n");
        printf("-P #      : server port #, default %s\n", server_port);
        printf("-d #      : clock divider #, default %u, (%u .. %u)\n", CLK_DIV_DEFAULT, CLK_DIV_MIN, CLK_DIV_MAX);        
        printf("-i [#,#]  : ctrl_in registers. with '0x' hex int. default [0,0]\n");
        printf("-o [#,#]  : ctrl_out registers. with '0x' hex int. default [0,0]\n");
        printf("-b <strb> : strobe delay r0:r1:r2:level (give 2x for both strobes)\n");
        printf("-p <1/2>  : primary (1) or secondary (0) board. default 1.\n");
        printf("-c <1/2>  : number of CPUs must be 1 or 2 (Cora-Z7-07S/-10). default 1.\n");
        printf("-w #      : waiting time before data generation. default 0.\n");
        printf("-f 0x#    : sync phase {ext,det} (hex number), default 0.\n");
...
//        printf("    -t # = run test number #\n");
    ...
    if(!err)
    {
        ...
        if (strb_delay == 0) {
            ...
            server = new FPGA_server(
                    flags, 
                    (flags & FLAG_SERVER) ? SERVER_LOCAL_IP : (address ? address : SERVER_GLOBAL_IP), 
                    port, 
                    clk_div,
                    ctrl_in,
                    ctrl_out,
